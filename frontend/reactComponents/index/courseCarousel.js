...
export default class CourseCarousel extends React.Component {
  state = {
    navigating: false
  };
                ...
                <div key={ i } className="inner-carousel">
                  ...
                  <Popover content={ popOverContent() } title={`Purchase this course for $${ course.publishedCourse.price }`}>
                    <Link to={ `/courses/view/${ course.publishedCourse._id }` }>
                      <Button onClick={ () => this.setState({ navigating: true }) } loading={this.state.navigating} style={{ background: course.publishedCourse.color }} className='explore-button' type="primary" icon="eye">
                        View Course
                      </Button>
                    </Link>
                  </Popover>
